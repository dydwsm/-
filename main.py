...
def earthquakeAnddeadload_response(pier_numlist, earth_style,filtered_df):
...
def earthquakeAnddeadload_response_dundi(pier_numlist, earth_style,filtered_df ):
...
import os
import shutil
file_name = "白沙洲引桥8x100m--墩宽4.5m--桩径2.0m--考虑支座摩阻力--0719--2.xlsx"
...
def main():
    ...
    写入到Excel中也是通过字典来实现的
    """
    pier_num = ['5#墩','6#墩','7#墩','8#墩','9#墩','10#墩','11#墩','12#墩','13#墩']
    dfs = {}
    filtered_df = []
    ...
    E1 = 'E1'
    E2 = 'E2'
    earthquake_response(b_list,E1, filtered_df)
    earthquake_response(b_list,E2, filtered_df)

    #输出承台底的内力响应表格
    earthquakeAnddeadload_response(b_list, E1, filtered_df)
    earthquakeAnddeadload_response(b_list, E2, filtered_df)
    # 输出墩底的内力响应表格
    earthquakeAnddeadload_response_dundi(b_list, E1, filtered_df)
    earthquakeAnddeadload_response_dundi(b_list, E2, filtered_df)
